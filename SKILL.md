...
> ⚠️ **版本基线**: 本 skill 的 API 描述已对齐到 **pi-coding-agent v0.85.1**（npm latest，2026-09-05 发布）。
>
...
createAgentSession()  ← 组装入口(Provider + 工具 + 资源)
       ...
       │
       └── 扩展(pi.on)                   ← 内部扩展层
             扩展独有(仅这 7 个 subscribe 收不到):
               context / tool_call / tool_result / before_agent_start / input / model_select / thinking_level_select
```

**关键区分(最大集成坑)**: 有 **7 个扩展独有事件**——`context` / `tool_call` / `tool_result` / `before_agent_start` / `input` / `model_select` / `thinking_level_select`。
在 server 层用 `session.subscribe` 监听这 6 个会**静默失败**(handler 被调用但 type 分支永不命中,无报错)。
**对策**: 需要抓这 7 个事件的逻辑(日志/trace/拦截/**抓这 7 个事件的数据落库**)必须写成扩展走 `pi.on`。⚠️ 但注意——**pi.on handler 被派发方 `await`，落库这类慢 I/O 必须 fire-and-forget**（推队列后台写），否则阻塞 agent loop。若只需存 `message_*`/`turn_*`/`tool_execution_*` 等 subscribe 收得到的事件，**落库优先走 `session.subscribe`**（不 await listener，可直接 await 写库，零阻塞）。详见 [04-events.md 关键细节](references/sdk_doc/04-events.md#关键细节)。
详见 [Common Mistakes](#common-mistakes) 与 [sdk_doc/04-events.md](references/sdk_doc/04-events.md)。

> ⭐ **推荐**：需要"agent 真正结束"信号时，订阅 **`agent_settled`** 替代 `agent_end`——前者保证所有 retry/compaction/queue 处理完才触发，每 prompt 一次，两层都派发。详见 [sdk_doc/04 坑 1](references/sdk_doc/04-events.md#坑-1不要把-agent_end-当成流程结束的唯一信号)。
...
- **从零实现/复刻一个 agent 引擎**(含其他语言 Rust/Go/Java),或评估一个 agent 的成熟度 → 逐模块对标 [agent-capability-checklist.md](references/agent-capability-checklist.md)(L1/L2/L3 分级,顶级标准以 pi v0.85.1 实测行为为基准)
...
**能力对标**：从零造引擎/评估 agent 成熟度时,逐模块过 [agent-capability-checklist.md](references/agent-capability-checklist.md)——纯 pi v0.85.1 基准的 12 个模块 × L1/L2/L3 分级(Provider/Loop/工具/事件/上下文/会话/扩展与能力注入/Subagent/集成/安全/可观测/评估),L3=pi 完整功能面(压缩参数 16384/20000、截断 2000 字符、subagent 并发 8/4、错误回喂、branch summary、SQLite 会话后端、export-html、pi evals 等),附事件流等价等 5 种验收方法。
...
> **未列出的场景**(终端 UI / 自定义命令 / Sandbox 等低频或 CLI 专属)直接翻 `references/scenarios/` 目录。**RPC 模式**(跨语言集成 Python/Go、进程隔离、把 pi 当子进程编排)本 skill 有意未覆盖——直接读官方 `node_modules/@earendil-works/pi-coding-agent/docs/rpc.md`(含 Extension UI Protocol 与双语言客户端示例);provider 请求拦截(`before_provider_request` / `before_provider_headers`)参考官方示例 `examples/extensions/provider-payload.ts`。
