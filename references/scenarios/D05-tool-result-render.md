...
> ⚠️ **关键集成坑 #1**：`tool_result` 是 **7 个扩展独有事件**之一（另六个是 `context` / `tool_call` / `before_agent_start` / `input` / `model_select` / `thinking_level_select`）。在 server 层用 `session.subscribe("tool_result", ...)` 会**静默失败**——handler 注册了但 type 分支永不命中。想做服务端结果后处理**必须走扩展机制**（`extensionFactories` + `pi.on`）。详见 [04-events.md 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
...
`tool_result` 是 7 个扩展独有事件之一（另六个是 `context` / `tool_call` / `before_agent_start` / `input` / `model_select` / `thinking_level_select`）。在 server 层用 `session.subscribe("tool_result", ...)` **静默失败**——handler 注册了但 type 分支永不命中，无任何报错。想做服务端结果后处理必须走扩展机制。详见 [04-events.md 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
