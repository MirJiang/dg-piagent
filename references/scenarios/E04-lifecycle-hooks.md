...
- 想改 systemPrompt 只能在创建时经 resourceLoader 整体覆盖（`systemPromptOverride`，见 [A03](A03-system-prompt.md)）——无法基于运行时动态信息
...
> ⚠️ **关键警示（扩展独有事件）**：`before_agent_start` 是 7 个**只在扩展层派发**的事件之一（另六个是 `context` / `tool_call` / `tool_result` / `input` / `model_select` / `thinking_level_select`）。在 server 层用 `session.subscribe("before_agent_start", ...)` 会**静默失败**——handler 注册了但永远不被调用，不报错不警告。想在 server 层感知 agent 启动只能用 `agent_start`（两层都派发，但拿不到 systemPrompt）。证据：types.ts、agent-session.ts。详见 [04-events 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
...
7 个扩展独有事件：`before_agent_start` / `context` / `tool_call` / `tool_result` / `input` / `model_select` / `thinking_level_select`。详见 [04-events.md 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
