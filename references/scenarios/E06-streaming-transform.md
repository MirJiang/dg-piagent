...
> ⚠️ **关键集成坑 1：`tool_result` 是 7 个扩展独有事件之一**（另六个是 `context` / `tool_call` / `before_agent_start` / `input` / `model_select` / `thinking_level_select`）。在 server 层用 `session.subscribe("tool_result", ...)` 会**静默失败**——外部事件流根本不派发这个 type。想做服务端工具结果后处理**必须走扩展**。详见 [04-events.md 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
...
**工具启用规则**（与 [A04](A04-tool-whitelist.md) / [A06](A06-load-extensions.md) / [D01](D01-custom-tool.md) / [D04](D04-confirm-destructive.md) / [D05](D05-tool-result-render.md) / [E01](E01-tool-intercept.md) 一致）：
...
if (!hookResult) return undefined;          // ← 没有 handler 返回修改时完全不变
return {
  content: hookResult.content,              // ← 四个字段都来自合并后的 currentEvent
  details: hookResult.details,              // handler 未覆盖时 = 原始 details（见下方说明）
  isError: hookResult.isError,
  usage: hookResult.usage,                  // handler 未覆盖时 = 原始 usage
};
```

> ✅ **字段合并语义（别被旧心智模型带偏）**：`emitToolResult` 在 runner 内维护 `currentEvent = { ...原事件 }`，handler 返回的对象**逐字段合并**进去（只覆盖显式给出的字段），最终四个字段全部从合并后的 `currentEvent` 取出。因此 **handler 只 return `{ content }` 不会丢失原始 `usage` / `details`**——未提及的字段自动保留原值。agent-loop 层还有一道 `afterResult.xxx ?? result.xxx` 兜底。只有当你**想改掉**某字段时才需要在返回值里显式给出。
...
// 注册顺序决定执行顺序：扩展经 DefaultResourceLoader 注入（createAgentSession
// 没有 extensions 选项），factory 数组顺序 = handler 执行顺序
const loader = new DefaultResourceLoader({
  cwd,
  agentDir: getAgentDir(),
  extensionFactories: [redactExtension, truncateExtension],  // 先脱敏，再截断
});
await loader.reload();
```
...
### 陷阱 6（★ 集成坑）：`tool_result` 是 7 个扩展独有事件之一
...
6. `model_select` — 模型选择
7. `thinking_level_select` — 思考等级变化（session 层对应事件是 `thinking_level_changed`）

**证据**：agent-session.ts 的 `AgentSessionEvent` 联合类型 不含上述 7 个 type。

想做服务端工具结果后处理**必须走扩展**（经 `DefaultResourceLoader` 的 `extensionFactories` 注册 handler——`createAgentSession` 没有 `extensions` 选项）。
...
- [E05 用户输入变换](E05-input-transform.md) — `input` 事件（7 个扩展独有事件之一），mutation 也无效
...
- [04-events 事件系统](../sdk_doc/04-events.md) — 7 个扩展独有事件清单、派发对比
