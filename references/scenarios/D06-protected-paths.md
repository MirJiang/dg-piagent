...
> ⚠️ **关键集成坑 1：`tool_call` 是 7 个扩展独有事件之一**（另六个是 `context` / `tool_result` / `before_agent_start` / `input` / `model_select` / `thinking_level_select`）。在 server 层用 `session.subscribe("tool_call", ...)` 会**静默失败**——外部事件流根本不派发这个 type。想做服务端路径限制**必须走扩展**。详见 [04-events.md 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
...
const cwd = "/data/sandbox/user123";
const loader = new DefaultResourceLoader({
  cwd,
  agentDir: getAgentDir(),
  extensionFactories: [
    createProtectedPathsExtension({
      allowedRoots: [cwd, "/tmp/agent-cache"],
    }),
  ],
});
// ⚠️ 传自定义 resourceLoader 时 createAgentSession 不会自动 reload（见 A06 坑 1）
await loader.reload();

const { session } = await createAgentSession({
  cwd,
  resourceLoader: loader,
  // model 可省略（走 models.json / 环境变量解析），也可显式传入
});
```
...
- [A04 工具白名单](A04-tool-whitelist.md) — 工具默认启用规则
