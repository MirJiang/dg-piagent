...
| 显式路径 | `additionalExtensionPaths: ["本地路径", "npm:包名", "https://…/repo.git"]`（git 来源须是完整 URL——`https://` / `ssh://` / `git://` 协议或 `git:` 前缀，不支持 `git+` 前缀简写，解析规则见 A06） | 扩展在其他位置 |
...
server 层 `session.subscribe("tool_call", ...)` 不会触发。要在扩展里订阅 `tool_call` 才能收到。7 个扩展独有事件：`context` / `tool_call` / `tool_result` / `before_agent_start` / `input` / `model_select` / `thinking_level_select`。**扩展独有 vs 双层 vs subscribe 独有的权威派发分类见 [04-events.md 坑 4](../sdk_doc/04-events.md)**，本场景只给概要。
