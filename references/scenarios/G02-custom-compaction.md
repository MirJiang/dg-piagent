...
> **派发层提醒**：`session_before_compact` / `session_compact` 是**扩展层 `pi.on` 事件**，外部宿主用 `session.subscribe` **收不到**。subscribe 层只能收到 `compaction_start` / `compaction_end`（不带 preparation 详情）。详见 [04-events.md 坑 4](../sdk_doc/04-events.md#坑-47-个扩展独有事件-subscribe-静默收不到--最大集成坑)。
