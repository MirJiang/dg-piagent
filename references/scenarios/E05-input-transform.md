...
- 顺序由 loader 的 `extensionFactories: [A, B, C]` 数组顺序决定
...
SDK 集成场景下扩展进程的 `process.cwd()` 不等于 session 的 `cwd`。`createAgentSession({ cwd })` 可选（默认 `process.cwd()`），handler 内取工作目录必须用 `ctx.cwd`（types.ts）。
