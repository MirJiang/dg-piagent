...
> ⚠️ **subscribe 静默收不到的 7 个事件**：`context` / `tool_call` / `tool_result` / `before_agent_start` / `input` / `model_select` / `thinking_level_select` —— 这些只在扩展层 `pi.on` 派发，**subscribe 监听这些 type 永远不会命中，且不报错**。详见 [sdk_doc/04-events.md 坑 4](../sdk_doc/04-events.md)。

> **本场景是外部宿主集成视角**（subscribe 转发到外部客户端），与 E06 的「扩展内部主动推送」不同。需要拦截/变换工具调用 → 走扩展层（[E01](E01-tool-intercept.md) / [E06](E06-streaming-transform.md)）。

## 默认行为

- `session.subscribe()` 不需要任何配置，每个 session 实例自带事件流
- 创建 session 时**不传 `tools`** 默认启用 `read` / `bash` / `edit` / `write` 四件套；`grep` / `find` / `ls`（及 Windows 下的 `powershell`）是内置工具但**非默认激活**，需显式列入 `tools` 才暴露给模型（与 A04 / D01 / D04 / D05 / E01 / E06 一致）
...
`AgentSessionEvent` 共 **24 种 distinct type**（源自 agent-session.ts，经 `coding-agent/src/index.ts` 对 SDK 使用者导出——类型签名可见且运行时会真实派发。源码里 `agent_end` / `summarization_retry_attempt_start` 各有 2 处声明，TS 合并后各算 1 种；v0.85.0 起新增 `bash_execution_update`）：
...
6. **subscribe 收不到 7 个扩展独有事件**（`context`/`tool_call`/`tool_result`/`before_agent_start`/`input`/`model_select`/`thinking_level_select`），写 type 分支永不命中且不报错。需要它们 → 走扩展层 `pi.on`。详见 [04-events 坑 4](../sdk_doc/04-events.md)。
