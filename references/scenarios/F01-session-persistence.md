...
2. **内存模式**：`SessionManager.inMemory()` 不写磁盘，`session.sessionFile` 为 `undefined`，`session.sessionManager.isPersisted()` 为 `false`；`cwd` 不传则默认 `process.cwd()`。**v0.85.0 起** inMemory 会话支持从外部管理的会话条目恢复（自己存库 → 恢复进内存会话，见 docs/sdk.md「Session Management」）
