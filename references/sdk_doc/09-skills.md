...
> **本文档针对 `@earendil-works/pi-coding-agent` 包**。`@earendil-works/pi-agent-core`包也有 `loadSkills` / `formatSkillsForPrompt` / `loadSkillsFromDir` 同名 API，但存在差异：agent 包的 `loadSkills` 是 **async**（返回 `Promise`）、签名不同（接收 `ExecutionEnv` + 目录列表）、`Skill` 接口字段也不同（见下方说明）。跨包使用请先核对签名。
...
> **⚠ 跨包陷阱（C01 教训复现点）**：以上是 `@earendil-works/pi-coding-agent` 包的 `Skill` 接口。`@earendil-works/pi-agent-core`包的 `Skill` 接口**完全不同**（`packages/agent/src/harness/types.ts:64-75`）：
