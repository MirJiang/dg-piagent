...
`@earendil-works/pi-coding-agent` 导出完整工具系统，支持使用内置工具（read / bash / edit / write / grep / find / ls / powershell——powershell 为 v0.84.3 新增的 Windows 可选工具）和注册自定义工具。
...
除了单工具工厂和已列出的 `createCodingTools` / `createReadOnlyTools`，内部模块 `core/tools/index.ts` 还实现了以下批量工厂。⚠️ **它们未从包根导出**（截至 v0.85.1），不能 `import ... from "@earendil-works/pi-coding-agent"`——需要批量 `ToolDefinition` 时请用上表根导出的 `createXxxToolDefinition` 逐个组装；下表仅供理解内部结构与源码定制参考：
...
| `createAllToolDefinitions(cwd, options?)` | `Record<ToolName, ToolDefinition>` | 全部 8 个工具的原始定义，以 Record 形式返回 |
...
> 其他内置工具（bash / powershell / edit / write / grep / find / ls）各自的 Options 接口（`BashToolOptions` / `EditToolOptions` 等）详见源码：
