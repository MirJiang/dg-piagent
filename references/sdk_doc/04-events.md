...
| `session_compact_failed` ⭐ v0.84.3 | 压缩失败或被中止 | 否 | 失败告警 / 监控（payload 含 reason、retry 状态、source、errorMessage） |
...
| `ui_prompt_start` / `ui_prompt_end` ⭐ v0.84.4 | 扩展开始 / 结束等待 `ctx.ui` 用户输入（如 confirm 弹框） | 否 | 宿主集成区分「模型在工作」与「在等用户确认」，精确计时与状态展示 |
...
> - **按需查源码**（未列出 payload）：模型事件、输入事件（`input` / `user_bash`）、资源事件（`resources_discover`）、9 种 `session_*` 事件（多数 CLI 专属）—— 见 `packages/coding-agent/src/core/extensions/types.ts`
>
