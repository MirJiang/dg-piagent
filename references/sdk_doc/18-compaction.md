...
| 监听事件 | `session.subscribe(...)` 监听 `AgentSessionEvent`：`compaction_start` / `compaction_end` / `summarization_retry_*` | `pi.on("session_before_compact", ...)` / `pi.on("session_compact", ...)`（可拦截/替换摘要） |
...
- **subscribe 层事件 ≠ 扩展层事件**。如果你用 `session.subscribe` 监听 `session_compact` 不会触发——`session_compact` 是扩展层事件（`pi.on`）。subscribe 层对应的是 `compaction_end`。
...
export default function (pi) {
  // handler 签名固定为 (args: string, ctx) => Promise<void>——注意 ctx 是第二个参数
...
```ts
// 扩展层事件（pi.on），不是 AgentSessionEvent。subscribe 层用 `session.subscribe` 监听 `compaction_end` 不等效。
...
> 这些事件用 `session.subscribe(listener)` 监听，与上面的扩展层事件**不同**。做 UI 反馈（进度条、错误提示）通常用这一层。
