...
`AgentSessionEvent` 共 **24 种**（注：源码 union 中 `agent_end` / `summarization_retry_attempt_start` 各声明两次，实际 distinct 类型 = 14 session 独有 + 10 来自 AgentEvent；v0.85.0 起新增 `bash_execution_update`）。事件分两类：
...
**B. session 独有（14 种，扩展层 `pi.on` 收不到）** ⭐
